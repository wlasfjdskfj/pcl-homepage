...
# ============ Hash 算法（和中间件里 djb2 完全一致） ============

def js_hash_code(s):
    """模拟 JS 里的 hashCode 函数"""
    h = 5381
    for ch in s:
        h = ((h << 5) + h) + ord(ch)
        h = h & 0x7fffffff  # 保持 32 位正整数
    return h


def deterministic_index(ip, date, salt, max_val):
    """模拟中间件的 deterministicIndex 函数"""
...
def check_ip_and_score():
    ...
    # 从 XAML 里提取人品分数（简单匹配 TextBlock Text="XXX" 中的分数）
    # 格式：<TextBlock Text="88" FontSize="52" ...
